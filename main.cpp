...
void Run()
{
    ...
    NN network({2,4,2}, {{0,0,0,0},{0,0}}); //passes number of nodes per layer (first layer is inputs) and biases
    ...
    //     pos.w = 4;
    //     pos.h = 4;
    //     SDL_SetRenderDrawColor(renderer, 255*training[i][0], 0, 255*training[i][1], 255);
    //     SDL_RenderFillRect(renderer, &pos);
    // }
    // SDL_RenderPresent(renderer);
    // vector<vector<double>> inputs = {{0,0},{1,0},{0,1},{1,1}};
    // vector<vector<double>> training {{1,0},{0,1},{0,1},{1,0}};
    cout << "Training..." << endl;
    double passes = 0;
    network.Pass({inputs[0]}, {training[0]});
    network.BackProp();
    for(int i = 0; i < 101000; i++){
        ...
        if(i % 100 == 0){
            Draw(inputs, network);
            for(int i = 0; i < inputs.size(); i++){
                pos.x = inputs[i][0]*125*4;
                pos.y = inputs[i][1]*125*4;
                pos.w = 4;
                pos.h = 4;
                SDL_SetRenderDrawColor(renderer, 255*training[i][0], 0, 255*training[i][1], 255);
                SDL_RenderFillRect(renderer, &pos);
        ...
        for(int m = 0; m < network.neuronLayers.size(); m++){
            for(int j = 0; j < network.neuronLayers[m].biase.size(); j++){
                for(int k = 0; k < network.neuronLayers[m].biase[j].size(); k++){
                    cout << network.neuronLayers[m].biase[j][k] << endl;
                    vector<double> temp = {network.neuronLayers[m].biase[j][k]};
                    if(temp.empty()){
                        i = 100999;
                    }
                }
...
void Draw(vector<vector<double>> inputs, NN network){
    for(double x = 0; x < 125; x++){
        for(double y = 0; y < 125; y++){
            ...
            if(num <= .5){
                network.Pass({{x/125,y/125}}, {{0}});
                pos.x = x*4;
                pos.y = y*4;
                pos.w = 4;
                pos.h = 4;
                SDL_SetRenderDrawColor(renderer, 255*network.output[0][0], 0, 255*network.output[0][1], 255);
                SDL_RenderFillRect(renderer, &pos);
            }
        }
    }
